import { WaveSetting, DownBackWaveSettings } from "./wave.config";
import { CalculateCurrentSetting } from "../globalAnimationSetting.config";

export const downBackWaveAnimation = (id: string) => {
...
function update(element: HTMLElement) {
  const setting: WaveSetting = CalculateCurrentSetting(DownBackWaveSettings);
